...
(function ($)
{
        ...
        this.each(function (i)
        {
            ...
            if (current.length > 0)
            {
                ...
                if (hash.length > 0)
                {
                    ...
                    var tabContainer = $('#' + hash), tabHidden = tabContainer.is(':hidden');
                    if (tabHidden)
                    {
                        tabContainer.show();
                    }
    ...
    $.fn.blockMessage.defaults = {type: 'info', position: 'top', animate: true, noMargin: true};
    ...
    $.fn.removeBlockMessages.defaults = {only: false, except: false, animate: true};
    $.fn.fold = function (duration, callback)
    {
        this.each(function (i)
        {
            ...
            var anim = {'height': 0, 'paddingTop': 0, 'paddingBottom': 0};
            ...
            element.stop(true).css({'overflow': 'hidden'}).animate(anim, {'duration': duration, 'complete': function ()
                {
                    $(this).css({'display': 'none', 'overflow': '', 'height': '', 'paddingTop': '', 'paddingBottom': '', 'borderTopWidth': '', 'borderBottomWidth': '', 'marginTop': '', 'marginBottom': ''});
                    if (callback)
                    {
                        callback.apply(this);
                    }
    ...
    $.fn.expand = function (duration, callback)
    {
        this.each(function (i)
        {
            ...
            element.stop(true).css({'overflow': '', 'height': '', 'paddingTop': '', 'paddingBottom': '', 'borderTopWidth': '', 'borderBottomWidth': '', 'marginTop': '', 'marginBottom': ''});
            var height = element.height();
            var paddingTop = parseInt(element.css('padding-top'));
            var paddingBottom = parseInt(element.css('padding-bottom'));
            var marginTop = parseInt(element.css('margin-top'));
            var marginBottom = parseInt(element.css('margin-bottom'));
            var css = {'overflow': 'hidden', 'height': 0, 'paddingTop': 0, 'paddingBottom': 0};
            var anim = {'height': height, 'paddingTop': paddingTop, 'paddingBottom': paddingBottom};
            if (!$.browser.msie || $.browser.version > 8)
            {
            ...
            element.stop(true).css(css).animate(anim, {'duration': duration, 'complete': function ()
                {
                    $(this).css({'display': '', 'overflow': '', 'height': '', 'paddingTop': '', 'paddingBottom': '', 'borderTopWidth': '', 'borderBottomWidth': '', 'marginTop': '', 'marginBottom': ''});
                    if (callback)
                    {
                        callback.apply(this);
                    }
    ...
    $.fn.addEffectLayer = function (options)
    {
        ...
        this.each(function (i)
        {
            ...
            layer.css({top: elementOffset.top + 'px', left: elementOffset.left + 'px'}).width(element.outerWidth()).height(element.outerHeight());
            var span = layer.children('span');
            var marginTop = parseInt(span.css('margin-top'));
            span.css({'opacity': 0, 'marginTop': (marginTop - 40) + 'px'});
            layer.css({'opacity': 0}).animate({'opacity': 1}, {'complete': function ()
                {
    ...
    $.fn.addEffectLayer.defaults = {message: 'Please wait...'};
    $.fn.loadWithEffect = function ()
    {
        this.addEffectLayer({message: $.fn.loadWithEffect.defaults.message});
        var target = this;
        var callback = false;
        ...
        args[index] = function (responseText, textStatus, XMLHttpRequest)
        {
            ...
            if (textStatus == 'success' || textStatus == 'notmodified')
            {
                ...
                span.css({'marginTop': currentMarginTop + 'px'}).animate({'opacity': 0, 'marginTop': (marginTop - 40) + 'px'}, {'complete': function ()
                    {
                        layer.fadeAndRemove();
                    }});
            }
    ...
    $.fn.loadWithEffect.defaults = {message: 'Loading...', errorMessage: 'Error while loading', retry: 'Retry', cancel: 'Cancel'};
...
(function ($)
{
    $.fn.addTemplateSetup(function ()
    {
        ...
        {
            $(this).children('.mini-menu').css('display', 'block').stop(true).animate({opacity: 0}, {'complete': function () {
                    $(this).css('display', '');
                }});
        });
        ...
        }).children('ul.reverted').addClass('keep-reverted');
    ...
    $(document).ready(function ()
    {
        ...
        if (!$(document.body).hasClass('special-page'))
        {
            ...
            }).tip({stickIfCurrent: true, offset: -3});
...
notify.defaults = {closeButton: true, autoClose: true, closeDelay: 8000};
(function ($)
{
    ...
    $.fn.showTip = function (options)
    {
        ...
        this.each(function (i)
        {
            ...
            if (!oldIE)
            {
                ...
                tip.stop(true).delay(settings.delay).animate({opacity: 1, top: position.top, left: position.left}, 'fast');
            }
        });
        return this;
    };
    ...
    function getTipPosition(element, tip, settings, animStart)
    {
        ...
        {
            case'right':
                return{top: Math.round(offset.top + (element.outerHeight() / 2) - (tip.outerHeight() / 2)), left: Math.round(offset.left + element.outerWidth() + (animStart ? settings.animationOffset + settings.offset : settings.offset))};
                break;
            case'bottom':
                return{top: Math.round(offset.top + element.outerHeight() + (animStart ? settings.animationOffset + settings.offset : settings.offset)), left: Math.round(offset.left + (element.outerWidth() / 2) - (tip.outerWidth() / 2))};
                break;
            case'left':
                return{top: Math.round(offset.top + (element.outerHeight() / 2) - (tip.outerHeight() / 2)), left: Math.round(offset.left - tip.outerWidth() - (animStart ? settings.animationOffset + settings.offset : settings.offset))};
                break;
            default:
                return{top: Math.round(offset.top - tip.outerHeight() - (animStart ? settings.animationOffset + settings.offset : settings.offset)), left: Math.round(offset.left + (element.outerWidth() / 2) - (tip.outerWidth() / 2))};
                break;
            }
    }
